...
**Esta rama (`18.0`) es la variante de instalación única**: un solo Odoo, una sola versión, en toda la máquina — nunca convive con otra versión en el mismo servidor (a diferencia de la rama `master`, que mantiene los scripts `-multi` para desarrollo/pruebas con varias versiones a la vez). Creada a partir de la rama `19.0` (que fue la primera en adaptarse), no de `master`. Por eso aquí las rutas y nombres **no llevan sufijo de versión**: `/opt/odoo` (no `/opt/odoo/18`), `odoo.conf`/`odoo.log` (no `odoo180.conf`), servicio `odoo` (no `odoo180`). La rama de Odoo/OCA a usar **no se pregunta**: los scripts de mantenimiento (`update_oca_upstream.sh`, `update_databases.sh`, `push_org_origin.sh`, `update_org_origin.sh`) la detectan automáticamente con `git -C /opt/odoo/odoo rev-parse --abbrev-ref HEAD`.
...
- **Rama de trabajo de este repo**: `18.0` (instalación única para Odoo 18.0; creada a partir de la rama `19.0`, que a su vez viene de `master` — que sigue manteniendo la variante `-multi` para desarrollo/pruebas).
...
- **Listas de configuración en `config/`**:
  ...
  - `pack_maralva_base.txt` — lista de módulos `depends` del pack maestro de esta versión (en `master` hay uno por versión, `pack_maralva_base18.txt`/`pack_maralva_base19.txt`; aquí solo el de la `18.0`, recuperado de `master:config/pack_maralva_base18.txt`). `scripts/pack-maker.sh` (el scaffolder que los consume) no está en esta rama — es una herramienta de desarrollo, se dejó fuera a propósito.
