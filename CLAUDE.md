...
- **`02-odoo-setup-multi.sh` — caso especial `openupgradelib`**: este repo de OCA no sigue el versionado por rama de Odoo, solo tiene `master`. Se clona aparte (mismo patrón fork→OCA→autofork que el resto, pero con `master` fijo en vez de `$BRANCH`), fuera del bucle de `reposoca.txt` — **no añadir `openupgradelib` a `reposoca.txt`**, rompería al intentar clonar una rama `$BRANCH` que no existe ahí.
