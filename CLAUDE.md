...
- **`scripts/pack-maker.sh`**: valida que nombre/versión/fichero de dependencias no vengan vacíos, y que el nombre técnico sea un módulo Python/Odoo válido (`^[a-z][a-z0-9_]*$` — minúsculas, dígitos, `_`, sin empezar por número); normaliza la versión (`18.0` → `18`) para que `/opt/odoo/<versión>/maralva-custom` siempre coincida con lo que genera `02-odoo-setup-multi.sh` (que usa solo el número); aborta si `$TARGET_DIR` ya existe, para no sobrescribir en silencio `README.md`/`__manifest__.py`/`res_company_data.xml`/`ir.model.access.csv` de un módulo ya generado; y ya no da el commit final por bueno a ciegas — si `git add`/`git commit` fallan (ej. `git config user.email` sin configurar en esa máquina), avisa explícitamente en vez de imprimir "✅" igualmente. El módulo en sí se genera siempre, aunque el commit falle.
